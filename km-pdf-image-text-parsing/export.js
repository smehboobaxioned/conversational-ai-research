...
// getDocument loads the PDF document and ops is used to get the list of operations
...
async function extractText(pdfPath) {
  ...
  for (let i = 1; i <= numPages; i++) {
    ...
    const sortedItems = content.items.sort((a, b) => b.transform[5] - a.transform[5] || a.transform[4] - b.transform[4]);
    const strings = sortedItems.map(item => item.str);
    text += strings.join(' ') + '\n';
  }
...
export { exportImages, extractText };
